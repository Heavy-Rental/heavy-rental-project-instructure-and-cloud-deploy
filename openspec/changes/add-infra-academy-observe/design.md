...
- CloudWatch Agent (LabRole may lack `logs:PutLogEvents`). Docker Engine `awslogs` is allowed via the instance profile; Ansible probes `CreateLogStream` and skips the driver if denied. `daemon.json` uses Engine log-opts (`awslogs-region`, `awslogs-group`, `tag`) — not ECS `awslogs-stream-prefix`. If dockerd rejects the file, revert to `json-file` and do not fail the play.
...
6. **Log groups** `/heavy-rental/{portal,rest,haystack,neo4j}`. Docker Engine `awslogs` (no CloudWatch Agent). Ansible probes `logs:CreateLogStream` on LabRole / `hr-paid-*` before switching the daemon log driver. Stream names use Engine `tag` (`{app}/{container name}`), not ECS `awslogs-stream-prefix`. Paid IAM still grants `CreateLogStream` and `PutLogEvents` so the driver can deliver events after the probe.
