...
- `APP_CORS_ALLOWED_ORIGINS` includes portal origin and `http://<rest_alb_dns>:8080` for **direct** REST ALB browser calls (ADR 0018). Portal nginx `/api` omits `Origin` (`proxy_set_header Origin ""`); same-origin login does not use that allow-list. If Origin is forwarded, `GET /api/auth/getBearerToken` is 403 `Invalid CORS request` and login is 401.
