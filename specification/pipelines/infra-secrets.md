...
| `heavy-rental/rest` | `asg-rest` / REST app CD | SoR `POSTGRES_*` + aliases + `SPRING_DATASOURCE_*`, `HAYSTACK_BASE_URL` (Haystack ALB), `APP_CORS_ALLOWED_ORIGINS` (`http://<portal_alb_dns>,http://<rest_alb_dns>:8080` — **direct** REST ALB browser calls; portal `/api` omits `Origin` and does not use this list), Stripe trio, `APP_JWT_SECRET` (env / reuse SM / generate ≥ 32), optional `ONEMAP_EMAIL` / `ONEMAP_PASSWORD` from Environment secrets |
