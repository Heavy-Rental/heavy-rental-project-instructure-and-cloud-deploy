...
- `sync-ssh-keys` writes `heavy-rental/ssh/bastion`, installs that public key on the bastion **and** the four app roles, and writes the matching **private** key only onto the bastion (`/home/ec2-user/.ssh/id_ed25519`). It also installs `hr-ssh-config`, which writes `~/.ssh/config` Host aliases for every running portal / REST / Haystack / Neo4j guest (`ssh portal`, `ssh rest-2`, `ssh haystack-1a`, …). App guests still never receive a PEM (ADR 0011).
...
- Operators: `scripts/bastion-connect.sh` prints SSM and optional ProxyJump commands. On the bastion, `hr-ssh-targets` lists Host aliases; `ssh portal` (etc.) hops with the bastion key. `.bashrc` refreshes aliases each login because app ASG IPs change.
